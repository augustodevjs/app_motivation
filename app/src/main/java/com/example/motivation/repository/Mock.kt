...
class Mock {
    ...
    fun getPhrase(value: Int, lang: String): String {
        if(value != all) {
            val filteredPhrases = listPhrases.filter {( it.categoryId == value ) && it.lang == lang}
            val index = Random.nextInt(filteredPhrases.size)
            return filteredPhrases[index].description
        }

        val filteredPhrasesLang = listPhrases.filter { it.lang == lang}
        val index = Random.nextInt(filteredPhrasesLang.size)
        return filteredPhrasesLang[index].description
    }
}